                ...
                <div class="course-card-full__price-box grid">
					...
					{if $_modx->isAuthenticated()}
						<form method="post" class="ms2_form">
							...
							<button class="course-card-full__btn" type="submit" name="ms2_action" value="cart/add">Купить</button>
						</form>
					{else}
						<form method="post" style="display: none;">
							<input type="hidden" name="options" value="[]">
						</form>
						<a data-fancybox data-src="#login-popup" href="javascript:;" class="course-card-full__btn" id="buyProductBtn">Купить.</a>
					{/if}
                </div>
            </div>
        </div>
	...
	document.querySelector('.course-card-about').addEventListener('click', function(evt){
		...
		if(addItem !== null){
			if(addItem.dataset.add == 'true'){
				...
				addItem.previousElementSibling.querySelector('.course-item__url').style.display = 'block';
			}
		}
	});
	{/if}
